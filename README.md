...
`netlify.toml` matters: `/audit/*` and `/lp/*` rules must stay before the
final SPA catch-all. The Calendly booking page lives at `/book`; the React
audit landing page is `/growth-audit` (the Dossier owns `/audit/`).
