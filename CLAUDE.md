...
**Campaign funnels co-hosted in this repo** (generated static files under `public/`,
URLs must not break): `/uk-recruitment/*`, `/case/what-a-client-costs/*` (generated by
`scripts/build-cases.mjs` from `cases/<slug>.json`) and `/g/<slug>/*` (generated by
`scripts/build-g.mjs` from `g/<slug>.json`). These are static on purpose: the React app
is client rendered, so a route there serves a 1.7 KB shell with no body copy and no
JSON-LD to anything that does not run JavaScript, which is fatal for an ad landing page
and for AI search. Generated output is committed; editing it by hand is lost on the next
build.
...
4. SEO: `usePageMeta` per page + `JsonLd` helper (`Service` schema on service pages,
   `FAQPage` on home). `public/sitemap.xml` is hand-maintained and must be updated when
   routes change. Static funnel pages carry their own `Organization` + `Service` JSON-LD
   emitted by their generator.
...
> Applies to the React site at `/` and everything rendered by it. It does **not**
> apply to the campaign funnels under `/g/`, `/geo-audit/` and `/lp/`, which each run
> a palette their own campaign was designed and approved in and are deliberately not
> harmonised into the brand gradient. `/g/uk-private-clinics` is paper `#F7F5F0` and
> ink `#10141C` with Fraunces and Geist, self-hosted; its tokens live in
> `scripts/build-g.mjs` and are enforced there (a build check rejects any green hex
> and any request to Google Fonts). Do not "correct" it to the dark theme.
